	...
	<view>
		...
		<view class="cart-bottom flex">
			...
			<view class="cart-bottom-btn-box flex-1">
				<view class="cart-bottom-btn" :class="{'noGoods': noGoods}" @click="toPreOrder(1)">去结算</view>
			</view>
		</view>
		<!-- 遮罩层 -->
			...
			<u-transition :show="cartShow" mode="fade-up">
				<view class="transition">
					...
					<scroll-view scroll-y="true" class="scroll-Y" @scroll="cartScroll">
						<template v-for="(item, index) in washSelectedGoods">
...
<script>
	...
	export default {
		...
		onShow() {
			this.goodsList = []
			this.washSelectedGoods = uni.getStorageSync('HJKJ-washSelectedGoods') || []
			this.noGoods = true
			if (this.washSelectedGoods.length) {
				this.noGoods = false
				this.calcCart()
			}
		...
		methods: {
			...
			fetchProduct() {
				...
				getGoodsList(data).then((res) => {
					...
					if (this.washSelectedGoods.length) {
						this.goodsList.map(item => {
							this.washSelectedGoods.map(items => {
								if (item.id == items.id) {
									item.num = items.num
								}
							})
			...
			cartAdd(goods) {
				...
				that.goodsList.find(function(item) {
					if (item.id == goods.id) {
						item.num += 1
						if (item.num == 1) {
							that.washSelectedGoods.push(item)
						} else {
							let index;
							that.washSelectedGoods.find(function(items, indexs) {
								if (items.id == goods.id) {
									index = indexs
								}
							})
							that.washSelectedGoods[index].num = item.num
						}
					}
				})
				this.calcCart()
				uni.setStorageSync('HJKJ-washSelectedGoods', this.washSelectedGoods)
				...
				that.goodsList.find(function(item) {
					if (item.id == goods.id) {
						...
						if (item.num == 0) {
							...
							that.washSelectedGoods.splice(index, 1)
						} else {
							let index;
							that.washSelectedGoods.find(function(items, indexs) {
								if (items.id == goods.id) {
			...
			cartDetailAdd(goods){
				this.washSelectedGoods.find(function(item) {
					if(goods.id == item.id){
						item.num += 1
					}
				})
				this.calcCart()
				uni.setStorageSync('HJKJ-washSelectedGoods', this.washSelectedGoods)
				this.washSelectedGoods = Object.assign([], this.washSelectedGoods)
			},
			cartDetailMinus(goods){
				let that = this
				this.washSelectedGoods.find(function(item, index) {
					if(goods.id == item.id){
						item.num -= 1
						if (item.num == 0) {
							that.washSelectedGoods.splice(index, 1)
			...
			calcCart() {
				...
				let discountPrice = 0
				let cartTotalNum = 0
				this.washSelectedGoods.forEach(function(item) {
			...
			toGoodsDetail(goods){
				uni.navigateTo({
					url: '/pages/laundryDetails/index?type=1&id=' + goods.id //跳转路径
				})
			},
			toPreOrder(type){
